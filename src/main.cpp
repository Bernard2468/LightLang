...
#include "backend/BytecodeFile.h"
...
int main(int argc, char* argv[])
{
    ...
    SourceContext sourceContext(sourceCode);

    Lexer lexer(sourceCode);
    vector<Token> tokens = lexer.tokenize();

    bool lexicalError = false;
    vector<Token> lexicalErrorTokens;
    ...
    for (const Token& token : tokens)
    {
        ...
        {
            lexicalError = true;
            lexicalErrorTokens.push_back(token);
        }
    }
    ...
    if (lexicalError)
    {
        ...
        for (const Token& token : lexicalErrorTokens)
        {
            cout << "Lexical error at line " << token.line;
            if (token.column > 0)
            {
                cout << ", column " << token.column;
            }
            cout << ": unrecognized input '" << token.lexeme << "'." << endl;
            cout << sourceContext.excerpt(token.line, token.column) << endl;
        }
    ...
    try
    {
        ...
        if (parser.hasErrors())
        {
            ...
            for (const ParseDiagnostic& diagnostic : parser.getErrors())
            {
                cout << diagnostic.message << endl;
                cout << sourceContext.excerpt(diagnostic.line, diagnostic.column) << endl;
            }
        ...
        if (!semanticSuccess)
        {
            ...
            const vector<string>& semanticErrors = semanticAnalyzer.getErrors();
            const vector<int>& semanticErrorLines = semanticAnalyzer.getErrorLines();

            for (size_t index = 0; index < semanticErrors.size(); index++)
            {
                cout << semanticErrors[index] << endl;

                if (index < semanticErrorLines.size())
                {
                    cout << sourceContext.excerpt(semanticErrorLines[index], 0) << endl;
                }
            }
