...
string IRGenerator::generateExpression(const Expr* expression)
{
    ...
    {
        // str(x) lowers to a unary conversion, not a call frame.
        if (call->getCallee() == "str")
        {
            if (call->getArguments().size() != 1)
                throw runtime_error("IR generation error: 'str' expects exactly one argument.");

            string operand = generateExpression(call->getArguments()[0].get());
            string temporary = newTemporary();
            code.emit({IROpCode::UNARY, temporary, operand, "", "str", "", "",
                       call->getLine()});
            return temporary;
        }
...
void IRGenerator::generateWhile(const WhileStmt* statement)
{
    ...
    // In a while loop 'continue' re-tests the condition, so the continue
    // target is the loop start.
    loopLabels.push_back({endLabel, startLabel});
...
void IRGenerator::generateFor(const ForStmt* statement)
{
    // Layout:
    //     initializer
    //   conditionLabel:
    ...
    //     body
    //   incrementLabel:            <- 'continue' lands here
    //     increment
    //     goto conditionLabel
    //   endLabel:                  <- 'break' lands here
    //
    // Placing the continue target before the increment is what keeps
    // 'continue' from skipping the update and looping forever.
    enterScope();
    ...
    code.emit({IROpCode::LABEL, "", "", "", "", "", conditionLabel, statement->getLine()});
    string condition = generateExpression(statement->getCondition());
    code.emit({IROpCode::IF_FALSE_GOTO, "", condition, "", "", "", endLabel, statement->getLine()});

    loopLabels.push_back({endLabel, incrementLabel});
    generateStatement(statement->getBody());
    loopLabels.pop_back();

    code.emit({IROpCode::LABEL, "", "", "", "", "", incrementLabel, statement->getLine()});
    generateStatement(statement->getIncrement());
    code.emit({IROpCode::GOTO, "", "", "", "", "", conditionLabel, statement->getLine()});
    code.emit({IROpCode::LABEL, "", "", "", "", "", endLabel, statement->getLine()});

    exitScope();
}

void IRGenerator::generateBreak(const BreakStmt* statement)
{
    if (loopLabels.empty())
        throw runtime_error("IR generation error: 'break' outside of a loop.");

    code.emit({IROpCode::GOTO, "", "", "", "", "",
               loopLabels.back().breakLabel, statement->getLine()});
}

void IRGenerator::generateContinue(const ContinueStmt* statement)
{
    if (loopLabels.empty())
        throw runtime_error("IR generation error: 'continue' outside of a loop.");

    code.emit({IROpCode::GOTO, "", "", "", "", "",
               loopLabels.back().continueLabel, statement->getLine()});
}
