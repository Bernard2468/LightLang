                          ...
                          ConstantValue& result) const
{
    ...
    if (operation == "str")
    {
        // str() of a string is the identity. For the other types the constant
        // text already matches the runtime rendering (formatFloat mirrors
        // RuntimeValue::toString), so folding cannot change program output.
        ...
        if (operand.type == ValueType::INT ||
            ...
            operand.type == ValueType::BOOL)
        {
            result = {ValueType::STRING, serializeString(operand.text)};
            return true;
        }
...
string Optimizer::expressionKey(const IRInstruction& instruction) const
{
    string key = instruction.operation;
    key += "#" + to_string(instruction.operand1.size()) + ":" + instruction.operand1;
    key += "#" + to_string(instruction.operand2.size()) + ":" + instruction.operand2;
    return key;
}

IntermediateCode Optimizer::eliminateCommonSubexpressions(
    const IntermediateCode& input) const
{
    // Local (basic-block) common subexpression elimination.
    //
    // Within one straight-line block, a repeated pure computation over
    // unchanged operands must produce the same value, so later occurrences are
    // dropped and their temporaries rewritten to the first one. Availability is
    // discarded at every label, jump, call, and function boundary, which keeps
    // the pass correct without a full dataflow analysis.
    //
    // Trapping operations stay safe: if the first evaluation would trap, the
    // program stops there and the eliminated duplicate was never reachable.
    unordered_map<string, string> available;
    unordered_map<string, string> rewrite;
    IntermediateCode output;

    for (IRInstruction instruction : input.getInstructions())
    {
        auto rewriteOperand = [&rewrite](string& operand)
        {
            auto found = rewrite.find(operand);
            if (found != rewrite.end()) operand = found->second;
        };

        rewriteOperand(instruction.operand1);
        rewriteOperand(instruction.operand2);
        ...
        if (instruction.opcode == IROpCode::BINARY ||
            instruction.opcode == IROpCode::UNARY)
        {
            ...
            {
                rewrite[instruction.destination] = found->second;
                continue;
            }

            available.emplace(key, instruction.destination);
            output.emit(instruction);
            continue;
        }

        // Any other instruction that writes a name invalidates every recorded
        // expression computed from that name.
        if (!instruction.destination.empty())
        {
            const string& written = instruction.destination;

            for (auto entry = available.begin(); entry != available.end();)
            {
                bool usesWritten =
                    entry->first.find("#" + to_string(written.size()) + ":" + written) !=
                    string::npos;

                if (usesWritten) entry = available.erase(entry);
                else ++entry;
            }
        }
