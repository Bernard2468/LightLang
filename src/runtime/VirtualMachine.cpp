    ...
    const vector<BytecodeInstruction>& instructions)
{
    switch (instruction.opcode)
    {
        case BytecodeOpCode::DECLARE:
        {
            ...
            // A declaration inside a loop body runs once per iteration, so
            // re-executing a DECLARE is normal and must reset the variable to
            // a fresh default rather than fail. Genuinely duplicated
            // declarations within one scope are already rejected during
            // semantic analysis, so no diagnostic is lost here.
            auto existing = frame.variables.find(instruction.operand);

            if (existing != frame.variables.end())
            {
                existing->second = VariableSlot{type, defaultValue(type)};
            }
            else
            {
        ...
        case BytecodeOpCode::TO_STRING:
        {
            // str(x). Reuses the same rendering as print, so that
            // print(str(x)) and print(x) always agree.
            RuntimeValue value = pop(instruction);
            if (value.getType() == ValueType::ERROR)
                runtimeError(instruction, "str() requires a valid operand.");
            push(RuntimeValue(value.toString()));
            instructionPointer++;
            break;
        }
