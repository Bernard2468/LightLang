...
ForStmt::ForStmt(unique_ptr<Stmt> initializerStatement,
                 unique_ptr<Expr> conditionExpression,
                 unique_ptr<Stmt> incrementStatement,
                 unique_ptr<Stmt> bodyStatement,
                 int sourceLine)
    : Stmt(sourceLine),
      initializer(move(initializerStatement)),
      condition(move(conditionExpression)),
      increment(move(incrementStatement)),
      body(move(bodyStatement)) {}
const Stmt* ForStmt::getInitializer() const { return initializer.get(); }
const Expr* ForStmt::getCondition() const { return condition.get(); }
const Stmt* ForStmt::getIncrement() const { return increment.get(); }
const Stmt* ForStmt::getBody() const { return body.get(); }
void ForStmt::print(int indentLevel) const
{
    indent(indentLevel);
    cout << "For" << endl;
    indent(indentLevel + 1);
    ...
    indent(indentLevel + 1);
    cout << "Increment" << endl;
    increment->print(indentLevel + 2);
    indent(indentLevel + 1);
    cout << "Body" << endl;
    body->print(indentLevel + 2);
}

BreakStmt::BreakStmt(int sourceLine) : Stmt(sourceLine) {}
void BreakStmt::print(int indentLevel) const
{
    indent(indentLevel);
    cout << "Break" << endl;
}

ContinueStmt::ContinueStmt(int sourceLine) : Stmt(sourceLine) {}
void ContinueStmt::print(int indentLevel) const
{
    indent(indentLevel);
    cout << "Continue" << endl;
}
