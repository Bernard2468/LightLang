...
//
// All three header clauses are required, which keeps the construct
// unambiguous and every loop guaranteed to have a termination test.
class ForStmt : public Stmt
{
private:
    unique_ptr<Stmt> initializer;
    unique_ptr<Expr> condition;
    unique_ptr<Stmt> increment;
    unique_ptr<Stmt> body;

public:
    ForStmt(unique_ptr<Stmt> initializerStatement,
            unique_ptr<Expr> conditionExpression,
            unique_ptr<Stmt> incrementStatement,
            unique_ptr<Stmt> bodyStatement,
            int sourceLine);

    const Stmt* getInitializer() const;
    const Expr* getCondition() const;
    const Stmt* getIncrement() const;
    const Stmt* getBody() const;
    void print(int indentLevel) const override;
};
...
{
public:
    explicit BreakStmt(int sourceLine);
    void print(int indentLevel) const override;
};
...
{
public:
    explicit ContinueStmt(int sourceLine);
    void print(int indentLevel) const override;
};
